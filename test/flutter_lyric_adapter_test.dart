...
void main() {
  group('flutter_lyric_adapter', () {
    test('convertToFlutterLyricModel accurately converts word-synced lyrics', () {
      ...
      final model = convertToFlutterLyricModel(result, showTransliteration: true);
      ...
      expect(line.words![0].start, equals(const Duration(milliseconds: 12000)));
      expect(line.words![0].end, equals(const Duration(milliseconds: 12400)));

      expect(line.words![7].text, equals('lights'));
      expect(line.words![7].start, equals(const Duration(milliseconds: 14900)));
    ...
    test('convertToFlutterLyricModel respects showTransliteration toggle', () {
      ...
      final withTrans = convertToFlutterLyricModel(result, showTransliteration: true);
      expect(withTrans.lines.first.translation, equals('Konnichiwa'));

      final withoutTrans = convertToFlutterLyricModel(result, showTransliteration: false);
    ...
    testWidgets('buildAppleMusicLyricStyle constructs valid Apple Music style', (tester) async {
        ...
        MaterialApp(
          home: Builder(
            builder: (context) {
              ...
              expect(rtlStyle.contentAlignment, equals(CrossAxisAlignment.end));
    ...
    test('LosslessMusicApi.normalizeTitle strips YouTube featuring and noise metadata', () {
      expect(LosslessMusicApi.normalizeTitle('Overdue (feat. Travis Scott)'), equals('overdue'));
      expect(LosslessMusicApi.normalizeTitle('Starboy [feat. Daft Punk]'), equals('starboy'));
      expect(LosslessMusicApi.normalizeTitle('Blinding Lights (Official Video)'), equals('blinding lights'));
      expect(LosslessMusicApi.normalizeTitle('Bohemian Rhapsody - Remastered 2011'), equals('bohemian rhapsody'));
    });
  });
}
